<template lang="html">
  <div class="m-colorPicker" ref="colorPicker" v-on:click="event => { event.stopPropagation() }">
    ...
    <!-- 颜色色盘 -->
    <div class="box" :style='position||{top:top,right:right,left:left,bottom:bottom}' v-bind:class="{ open: openStatus }">
...
export default {
  ...
  data () {
    return {
      ...
      // 标准颜色 
      bColor: [
        '#ffffff', '#f2f2f2', '#f2f3f5', '#ede3fd', '#f6f6f6', '#e6e6e6', '#e0e1e3', '#dddddd', '#d4d4d4', '#b2b5ba', '#fff3bb', '#ffdcdc', '#dee9f9', '#fdeee8', '#ceebe6', '#d8efff', '#e0f5f4', '#bad2f2', '#d1d6f6', '#BBBBBB', '#8e8e8e', '#666666', '#798093', '#001c68', '#4d4d4d', '#015442', '#abe66c', '#3c7800', '#262626', '#0653b6', '#4628AA', '#32AAAA', '#1c69d4', '#3b569d', '#E02020', '#fa6400', '#d26e50',],
      //  
  ...
  methods: {
    clickfun (event) {
      ...
      event.clientY > 275 ? this.bottom = '40px' : this.bottom = ''
      event.clientY < 275 ? this.top = '40px' : this.top = ''
      document.documentElement.clientWidth - event.clientX > 250 ? this.right = '' : this.right = '-90px'
      document.documentElement.clientWidth - event.clientX < 250 ? this.left = '' : this.left = '-90px'
    },
    triggerHtml5Color () {
